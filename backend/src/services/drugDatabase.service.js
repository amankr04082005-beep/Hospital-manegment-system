 ...
 *     the Drug Database API (drug-database.com) using substance -> ATC
 *     -> drugs-in-ATC-subtree lookup.
 *
...
// Drug Database (drug-database.com) integration.
...
/**
 * --- Drug Database (drug-database.com) alternatives lookup ---
 * Flow: substance name -> UNII -> ATC code -> drugs in that ATC subtree.
 * Used as a fallback when no local alternatives are found.
 * Note: country coverage for India is currently empty on this provider;
 * we default to 'CH' where the provider actually has data, purely so
 * this fallback returns something rather than nothing. Results should
 * be clearly labeled as external/foreign in the UI.
 */

async function resolveSubstanceToUnii(substanceName) {
...
async function getSubstanceAtcCode(unii) {
  if (!unii || !DRUG_DB_API_KEY) return null;
  try {
    const { data } = await axios.get(`${DRUG_DB_BASE_URL}/v1/substances/${unii}`, {
      headers: { Authorization: `Bearer ${DRUG_DB_API_KEY}` },
      timeout: 10000,
    });
    return data?.atcs?.[0]?.code || null;
  } catch (err) {
    console.error('Drug Database substance details failed for', unii, '-', err.response?.status || err.message);
    return null;
  }
}
...
async function findAlternativesFromDrugDatabase(substanceName, countryCode = 'CH') {
  ...
  const unii = await resolveSubstanceToUnii(substanceName);
  if (!unii) return [];

  const atcCode = await getSubstanceAtcCode(unii);
  if (!atcCode) return [];
